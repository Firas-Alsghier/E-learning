...
const unblockStudent = async (id: string) => {
  const confirmUnblock = confirm('Are you sure you want to unblock this student?');
  if (!confirmUnblock) return;

  try {
    await api.patch(`/students/${id}/unblock`);

    // Remove from blocked list instantly
...
<template>
  <div class="space-y-6">
    <h1 class="text-2xl text-center mt-6 font-semibold">Blocked Students</h1>
        ...
        <tbody>
          <template v-for="student in students" :key="student._id">
            <tr class="border-t text-center">
              ...
              <td class="p-4">
                <span class="px-2 py-1 rounded text-xs font-medium bg-red-100 text-red-700"> Blocked </span>
              </td>
              ...
              <td class="p-4 flex gap-2 justify-center">
                ...
                <Button class="cursor-pointer" size="sm" variant="outline" @click="unblockStudent(student._id)"> Unblock </Button>
          ...
          <tr v-if="students.length === 0">
            <td colspan="5" class="p-6 text-center text-muted-foreground">No blocked students found.</td>
          </tr>
        </tbody>
      </table>
    </div>
