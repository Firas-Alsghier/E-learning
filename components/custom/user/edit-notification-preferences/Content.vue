  ...
  <form class="space-y-8 bg-[#f9f9f9] p-6 rounded-xl">
    ...
    <div class="border rounded-lg overflow-hidden">
      <div class="flex items-center justify-between p-4 border-b">
        <Switch v-model:checked="offers.value.enabled" />
        <h2 class="text-lg font-semibold">التحديثات والعروض</h2>
      </div>
      <div class="space-y-2 px-4 py-3">
        <div class="flex items-center justify-end gap-2">
          <Checkbox v-model:checked="offers.value.productUpdates" />
          <label>إطلاق المنتجات والإعلانات</label>
        </div>
        <div class="flex items-center justify-end gap-2">
          <Checkbox v-model:checked="offers.value.promotions" />
    ...
    <div class="border rounded-lg overflow-hidden">
      <div class="flex items-center justify-between p-4 border-b">
        <Switch v-model:checked="learning.value.enabled" />
        <h2 class="text-lg font-semibold">التعلم الخاص بك</h2>
      </div>
      <div class="space-y-2 px-4 py-3">
        <div class="flex items-center justify-end gap-2">
          <Checkbox v-model:checked="learning.value.stats" />
          <label>إحصاءات التعلم</label>
        </div>
        <div class="flex items-center justify-end gap-2">
          <Checkbox v-model:checked="learning.value.inspiration" />
          <label>الإلهام (النصائح والقصص وما إلى ذلك)</label>
        </div>
        <div class="flex items-center justify-end gap-2">
          <Checkbox v-model:checked="learning.value.courseRecs" />
          <label>توصيات الدورة</label>
        </div>
        <div class="flex items-center justify-end gap-2">
          <Checkbox v-model:checked="learning.value.lecturerNotifs" />
